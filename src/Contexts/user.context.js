...
export const UserProvider = ({children}) => {
    ...
    const [user, setUser] = useState(null);
     ...
     // Function to log in user into our App Service using their email & password
     const emailPasswordLogin = async (email, password) => {
        const credentials = Credentials.emailPassword(email, password);
        const authenticated_User = await app.logIn(credentials);
      ...
      const emailPasswordSignup = async (email, password) => {
       ...
       const fetchUser = async () => {
        ...
        try {
            ...
            // Now, if we have a user, we are setting it to our user context so that we can use it in our app across different components.
        ...
        // Function to logout user from our App Services
